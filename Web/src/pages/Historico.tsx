...
import  ListItem from "../components/historico/ListItem";
    ...
    useEffect(() => {
        const fetchHistorico = async () => {
            try {
                ...
                const questoes: Questao[] = historicoData.map((h) => h.questao);
                setData(questoes);
              ...
              .filter((q) =>
                ...
                <div className="flex flex-col gap-4 w-full">
                  {filteredItems.map((item) => {
                        // encontra o registro do histórico referente a esta questão
                        const histItem = historico.find(h => h.questao.id === item.id);

                        return (
                            <ListItem
                                key={item.id}
                                desc={item.texto}
                                disciplina={item.assunto.disciplina.nome}
                                assunto={item.assunto.nome}
                                dificuldade={item.dificuldade}
                                resolvida={histItem?.resolvida ?? false}
